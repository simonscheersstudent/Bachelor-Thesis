...
X_all <- sparse.model.matrix(~. -1,data=as.data.frame(feature_engineered_dt3))
...
feature_engineered_dt3$predictions_xgb <- predictions_xgb_all
...
train_new <- copy(feature_engineered_dt3)
...
dt_new<-get_personalized_models(train_new)
...
write_csv(player_rgax,paste0(getwd(), "/final_output/rgax_output.csv"))
