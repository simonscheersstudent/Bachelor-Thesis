...
get_personalized_models <- function(input){
  results <- data.table()
  ...
  for (shooter in unique(input$shooter_id)){
    ...
    #print(str(df))
    'rf <- ranger(formula = chosen_shooter ~ .-predictions_xgb,
                 data= df,
                 probability = TRUE,
                 num.trees = 1000,
                 min.node.size = 5,
...
dt_new3<-get_personalized_models(train_new)

dt_new3 <- merge(dt_new3, player_info[, c(-2,-3, -4,-5, -8, -9,-10,-11,-12, -13,-15,-16)], by.x = "shooter", by.y = "player.id", all.x=TRUE)
setnames(dt_new3, c("nickname","positionGroupType.x"), c("shooter_name", "shooter_position_type"))
rgax_data_final_with_shots <- dt_new3

#sum(train_new[shooter_id==1,predictions_xgb])
'dt_new$goal_numeric <- as.numeric(dt_new$goal)-1
dt_new$chosen_shooter_numeric <- as.numeric(dt_new$chosen_shooter)-1

dt_new$residual_goal_prob <- dt_new$goal_numeric-dt_new$predictions_xgb
dt_new$residual_shot_prob <- dt_new$chosen_shooter_numeric - dt_new$shot_expectations

### rgax implementation
rgax_model <- lm(residual_goal_prob ~ residual_shot_prob, data= dt_new)
summary(rgax_model)

player_rgax <- dt_new[, .(rgax = mean(residual_goal_prob * residual_shot_prob / mean(residual_shot_prob^2))),by= .(shooter_id)]
'
